...
window = tk.Tk()
window.title("Scale & Spinbox demo")
window.geometry("320x220")

# --- Scale (slider) --------------------------------------------------------
tk.Label(window, text="Volume").pack(pady=(14, 0))
vol_label = tk.Label(window, text="0", font=("TkDefaultFont", 14))
vol_label.pack()
...
# --- Spinbox ---------------------------------------------------------------
tk.Label(window, text="Quantity (1-10)").pack(pady=(14, 0))
qty = tk.IntVar(value=1)
tk.Spinbox(window, from_=1, to=10, textvariable=qty, width=5).pack()

def confirm():
    # qty.get() is already an int because we used an IntVar.
    vol_label.config(text=f"Qty {qty.get()}")

tk.Button(window, text="Confirm quantity", command=confirm).pack(pady=10)
