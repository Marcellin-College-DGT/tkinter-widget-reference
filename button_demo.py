...
status = tk.Label(window, text="Press a button", font=("TkDefaultFont", 13))
status.pack(pady=16)
...
hello_btn = tk.Button(window, text="Say hello", command=say_hello)
...
for colour in ("red", "green", "blue"):
    tk.Button(window, text=colour, command=lambda c=colour: choose(c)).pack(pady=2)
...
toggle_btn = tk.Button(window, text="Disable 'Say hello'", command=toggle_hello)
